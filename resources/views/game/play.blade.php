...
<table>
    ...
    @foreach ($gameData as $data)
        <tr><td>{{$data->pid}}</td><td><a href="{{url('play/?mid=' .$data->mid)}}">{{$data->mname}}</a></td><td><a href="{{url('play/?gid=' .$data->gid)}}">{{$data->gname}}</a></td><td>{{$data->hi}}</td><td>{{$data->evaluation}}</td></tr>
    @endforeach
</table>
